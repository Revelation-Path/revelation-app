        ...
        <div>
            ...
            {parsed.sections.into_iter().map(|section| {
                let section_label: Option<&str> = match section.section_type {
                    SongSectionType::Chorus => Some("Припев"),
                    SongSectionType::Verse => section.label.as_ref().map(|_| "Куплет"),
                    SongSectionType::Bridge => Some("Бридж"),
                    SongSectionType::PreChorus => Some("Предприпев"),
                    _ => None
                };
...
fn render_chords_line(text: &str, chords: &[PositionedChord]) -> String {
